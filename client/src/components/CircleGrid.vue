    ...
    <v-row v-if="!loading && circles.length > 0">
      <v-col class="pa-1" md="3" sm="4" cols="6" v-for="circle in circles" :key="circle.name">
        <v-card :to="'/' + circle.name"
          ...
          <v-img class="mt-2" style="background-color: lightgray" height="100%" :src="circle.imageUri" cover>
          ...
          <v-icon v-if="circle.favorited" size="24" class="favorite-heart">
            mdi-heart
          </v-icon>
        </v-card>
        <v-btn
          v-if="circle.circleAccess?.acceptTarget === 'ACCEPT_TARGET_RECIPIENT' && circle.circleAccess?.state === 'ACCESS_STATE_PENDING'"
          color="success" class="accept-btn" @click.stop.prevent="$emit('accept', circle)" block
          :loading="acceptingCircleId === circle.name">
          Accept
        </v-btn>
        <v-btn
          v-if="circle.circleAccess?.acceptTarget === 'ACCEPT_TARGET_RECIPIENT' && circle.circleAccess?.state === 'ACCESS_STATE_PENDING'"
          color="error" class="decline-btn" @click.stop.prevent="$emit('decline', circle)" block>
          Decline
        </v-btn>
      </v-col>
    </v-row>
