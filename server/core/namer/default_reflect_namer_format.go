...
func (n *defaultReflectNamer[T]) MustFormat(in T, options ...formatReflectNamerOption) string {
...
func (n *defaultReflectNamer[T]) Format(in T, options ...formatReflectNamerOption) (string, error) {
	...
	// Try each pattern in order; return the first successful format
	var lastErr error
...
func (n *defaultReflectNamer[T]) extractPatternValues(patternDetails patternDetails, in T) ([]string, error) {
	var vars []string
...
func formatFieldValue(pv patternKeyDetails, in any) (string, error) {
	// Get the field from the input struct using the index path
	field := reflect.ValueOf(in).FieldByIndex(pv.fieldIndexPath)
