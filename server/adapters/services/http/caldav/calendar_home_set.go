...
func (s *Service) CalendarHomeSet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.log.Error().Err(err).Msg("Failed to parse userID in CalendarHomeSet")
	...
	if err != nil {
		s.log.Error().Err(err).Msg("Failed to parse auth data in CalendarHomeSet")
		w.WriteHeader(http.StatusUnauthorized)
		return
	}

	if userID != authAccount.AuthUserId {
		s.log.Error().Int64("userID", userID).Int64("authUserID", authAccount.AuthUserId).Msg("UserID does not match authUserID in CalendarHomeSet")
