...
func (c *Client) GetFileContents(ctx context.Context, location string) (io.ReadCloser, error) {
	...
	if resp.ContentLength > maxDownloadSize {
		return file.File{}, errz.NewInvalidArgument("the file at %s was too large", location)
	}

	if resp.ContentLength < 0 {
		return file.File{}, errz.NewInvalidArgument("the file size at %s was unkonown", location)
	}
